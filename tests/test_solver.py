...
def test_trivial_constraints():
    ...
    solver = SpaceAllocator(((-1, -1), (3, 3), (5, 5), (7, 7)))
...
def test_constraint_validation():
    ...
    with pytest.raises(errors.SolverError):
        SpaceAllocator(((0, -4), (2, 3), (None, 5)))

    with pytest.raises(errors.SolverError):
        SpaceAllocator(((-4, 0), (3, 2), (None, 5)))

    # fixing order of constraints stops error
    SpaceAllocator(((-4, 0), (2, 3), (None, 5)))


# actual solver tests
def test_solution_bounds():
    # both bounds
    allocator = SpaceAllocator(((3, 5), (2, 50), (9, 15)))
    assert allocator.lower_bound == 3 + 2 + 9
    assert allocator.upper_bound == 5 + 50 + 15

    # no lower bound
    allocator = SpaceAllocator(((None, 5), (2, 50), (9, 15)))
    assert allocator.lower_bound == None
    assert allocator.upper_bound == 5 + 50 + 15

    # no upper bound
    allocator = SpaceAllocator(((3, 5), (2, None), (9, 15)))
    assert allocator.lower_bound == 3 + 2 + 9
    assert allocator.upper_bound == None

    # no bounds
    allocator = SpaceAllocator(((None, 5), (2, 50), (9, None)))
    assert allocator.lower_bound == None
    assert allocator.upper_bound == None
...
def test_hash():
    ...
    assert SpaceAllocator(bounds[0]) == SpaceAllocator(bounds[0])
    # solvers with different parameters evaluate to not be equal
    assert SpaceAllocator(bounds[0]) != SpaceAllocator(bounds[1])
